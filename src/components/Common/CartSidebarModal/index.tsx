...
//     onClick={() => closeCartModal()}
//     href="/cart"
//     className="w-full flex justify-center font-medium text-white bg-[#A855F7] py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#7C3AED]"
//   >
//     View Cart
//   </Link>
...
//     href="/checkout"
//     onClick={() => closeCartModal()}
//     className="w-full flex justify-center font-medium border border-[#A855F7] text-[#A855F7] bg-white py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#A855F7] hover:text-white"
//   >
//     Checkout
//   </Link>
// </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-end h-full">
        <div className="modal-content w-full max-w-[500px] h-full shadow-1 bg-white px-4 sm:px-7.5 lg:px-11 flex flex-col">

          {/* Header — fixed at top */}
          <div className="flex items-center justify-between pb-7 pt-4 sm:pt-7.5 lg:pt-11 border-b border-gray-3 shrink-0">
            <h2 className="font-medium text-dark text-lg sm:text-2xl">
              Cart View
            </h2>
            <button
          ...
          <div className="flex-1 h-0 overflow-y-auto no-scrollbar py-7.5">
            <div className="flex flex-col gap-6">
              {cartItems.length > 0 ? (
                cartItems.map((item, key) => (
                  <SingleItem
          ...
          <div className="border-t border-gray-3 bg-white pt-5 pb-6 shrink-0">
            <div className="flex items-center justify-between gap-5 mb-6">
              <p className="font-medium text-xl text-dark">Subtotal:</p>
              <p className="font-medium text-xl text-dark">${totalPrice}</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <Link
                onClick={() => closeCartModal()}
                href="/cart"
                className="w-full flex justify-center font-medium text-white bg-[#7C3AED] py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#6D28D9]"
              >
                View Cart
              </Link>
              <Link
                href="/checkout"
                onClick={() => closeCartModal()}
                className="w-full flex justify-center font-medium border border-[#7C3AED] text-[#7C3AED] bg-white py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#7C3AED] hover:text-white"
              >
                Checkout
              </Link>
            </div>
