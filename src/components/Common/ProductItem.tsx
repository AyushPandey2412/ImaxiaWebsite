...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      {/* ── Image container ── */}
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#EEE6FF] border border-[#DDD0FF] min-h-[270px] mb-4">

        <Link href={`/products/${item.id}`} className="relative w-full h-full aspect-square block">
          <Image
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-dark bg-white hover:text-[#7C3AED]"
          >
          ...
          <button
            ...
            // className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-[#7C3AED] text-white ease-out duration-200 hover:bg-[#6D28D9]"
            className={`inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px]
${isInCart ? "bg-[#A855F7] text-white" : "bg-[#A855F7] text-white hover:bg-[#7C3AED]"}
  ease-out duration-200`}
      ...
      <div className="flex items-center gap-2.5 mb-2">
        <div className="flex items-center gap-1">
          {[...Array(5)].map((_, idx) => (
            <Image key={idx} src="/images/icons/icon-star.svg" alt="star" width={14} height={14} />
          ))}
        </div>
      ...
      {/* Title */}
      <h3 className="font-medium text-dark ease-out duration-200 hover:text-[#7C3AED] mb-1.5">
